...
To find similar products in order to fill `similar products` list we run a query that resolve the intersection of the tags arrays, between the `current product` (the one that we are viewing its detail) and the others in the database. Products that have more matches go first and the ones that have less matches go last. Lets call this query `similarity query`.
